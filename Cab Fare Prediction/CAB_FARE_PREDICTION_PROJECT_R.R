rm(list=ls(all=T))
setwd("C:/Users/Mujeeb/Downloads/DATA SCIENTIST/PROJECT/PROJECT 2")
...
# Creating a function to calculate missing values ###################
...
#Since PAssenger_count is a categorical Variable , so we will use mode for Imputation
...
# We found that KNN is giving the value closest to Actual Value, We choose KNN for missing value imputation
...
# We have to Remove negative values from 'fare_amount'
...
#replacing all other missing value with mean
...
write.csv(pred_data, "Predicted_TestData.csv", row.names = F)
